...
#let tuda-dr-text(ty) = {
  ...
  if lt == "rernat" {
    ...
    [Zur~Erlangung~des~Grades~eines~Doctor~rerum~politicarum (Dr. rer. pol.)]
  } else {
    panic("Unknown dr type " + lt)
  }
}
...
#let tudpub-make-title-page(
  ...
  let (thesis_type_text, date_text, reviewers) = {
    if type(thesis_type) == str {
      ...
      let dr-txt = tuda-dr-text(thesis_type.at("dr"))
      let birth-txt = if birthplace == none {
        none
      } else {
        [ aus #birthplace]
      }
      let txt = [#dr-txt\ Vorlegete Dissertation von #author#birth-txt]
      if date_of_exam == none {
