...
#let get-spacing-zero-if-first-on-page(
  ...
  // note: this is a hack
  let heading_is_first_el_on_page = (
    heading_location.position().y <= content_page_margin_full_top
  )
...
#let check-font-exists(font-name) = {
  ...
  if measured.width == 0pt [
    #rect(stroke: (paint: red), radius: 1mm, inset: 1.5em, width: 100%)[
      ...
      Please install the required font "#font-name". For instructions see the
      #link(
        "https://github.com/JeyRunner/tuda-typst-templates#logo-and-font-setup",
